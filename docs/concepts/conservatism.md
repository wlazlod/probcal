...
pooled sets, a run that fails coverage for any grade also fails it for the binding
grade, so the joint (all-grades) event and the minimum single-grade event
happen to line up here. That is not a general identity, just what this configuration
produces.
