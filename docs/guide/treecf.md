...
- Provenance: the certificate's calibrated-target block carries a
  ...
  buffer_logit}`, where the fingerprint comes from the duck-typed
  `fingerprint()` (available on every probcal object) and is `null` when
  absent. Every `BatchRecord` carries `calibrator_fingerprint`.
...
- Plateau-aware feasibility tests: an engine-level plateau suite against
  brute force, run twice, once with a counting stub implementing probcal's
  generalized-inverse step contract and once with real probcal
...
- Test matrix: a matrix test module in treecf with probcal as an
  ...
  search) could flip through many trees. In the reproducing case the exact
  backend stamped `proof="optimal"` on an `x_cf` whose true
  ...
  0.2.4, which the `probcal[treecf]` extra pins. XGBoost also casts
