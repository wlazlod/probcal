...
generation. First, on the affine class the two modes agree to numerical precision, and the
...
Johansson and Sönströd, 2024) builds uncertainty-aware explanations on Venn–Abers
foundations, a different object (explanations with uncertainty semantics) than the array-level
